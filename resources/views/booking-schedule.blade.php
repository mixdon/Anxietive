    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-10">
        ...
        <div>
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <button id="toForm" class="w-full mt-6 bg-gray-800 text-white py-2 rounded-md disabled:opacity-50" disabled>
                    Next
                </button>
            </div>
        </div>
...
(function(){
    ...
    function checkToForm(){
        ...
        if(toFormBtn.disabled){ toFormBtn.classList.add('opacity-50'); }
        else{ toFormBtn.classList.remove('opacity-50'); }
    }
    ...
    function renderCalendar(date){
        ...
        for(let d=1;d<=daysInMonth;d++){
            const btn=document.createElement('button');
            btn.type='button'; btn.textContent=d; btn.className='py-2 rounded-md';
            const thisDate=new Date(year, month, d);
            const today=new Date(); today.setHours(0,0,0,0);

            if(thisDate<today){ 
                btn.classList.add('text-gray-300'); btn.disabled=true; 
            } else {
                btn.classList.add('hover:bg-green-100','cursor-pointer');
                btn.addEventListener('click', function(){
                    selectedDate=thisDate; selectedTime=null;
